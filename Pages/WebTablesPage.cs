...
namespace demosite.Pages
{
    public class WebTablePage
    {
        ...
        public async Task Addrecord(string firstName, string lastName, string userName, int age, int salary, string department)
        {
            await ResilienceHelper.SafeClick(page, "#addNewRecordButton");

            await ResilienceHelper.SafeFill(page, "#firstName", firstName);
            await ResilienceHelper.SafeFill(page, "#lastName", lastName);
            await ResilienceHelper.SafeFill(page, "#userEmail", userName);
            await ResilienceHelper.SafeFill(page, "#age", age.ToString());
            await ResilienceHelper.SafeFill(page, "#salary", salary.ToString());
            await ResilienceHelper.SafeFill(page, "#department", department);

            await ResilienceHelper.SafeClick(page, "#submit");
        }
